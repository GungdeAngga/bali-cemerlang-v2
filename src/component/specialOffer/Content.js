...
import { baseStorage, getOffer } from "../util/api/api";
      ...
      <div className="mt-6 lg:w-full lg:mt-9">
        <Slider {...settings}>
          {dataOffer.map((data) => (
            <div
              key={data.id}
              className="bg-white h-[200px] md:h-[270px] lg:h-[390px] text-black rounded-xl"
            >
              <div className="h-[95px] md:h-36 lg:h-56 flex justify-center items-center md:bg-VividRed rounded-t-xl">
                <img
                  src={baseStorage + data.foto_offer}
                  alt=""
                  className="h-full w-full md:h-32 md:w-32 lg:h-52 lg:w-52 object-cover rounded-t-xl md:rounded-full"
                />
              </div>
              ...
              <div className="flex flex-col justify-center items-center gap-1 p-2 lg:gap-4 lg:p-4">
                ...
                <p className="text-xs md:text-sm lg:text-base text-justify px-3 h-5 md:h-10">
                  {data.id
                    ? data.isi_offer
                    : `${data.isi_offer.slice(0, 50)}...`}
                </p>
                <Link to="/SpecialOfferDetalisPage">
                  <button className="bg-VividRed text-white text-xs md:text-base lg:text-lg px-6 py-1 mt-3 md:mt-1 rounded-xl">
                    Read More
                  </button>
                </Link>
              </div>
